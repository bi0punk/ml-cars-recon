    ...
    def draw_detections(self, frame, detections):
        ...
        for detection in detections:
            ...
            if (zona_central_x[0] <= cx <= zona_central_x[1] and 
                ...
                # NUEVO BLOQUE: captura por vehículo
                id_vehiculo = f"{x1}_{y1}_{x2}_{y2}"
                ahora = time.time()
                ultima = self.ultima_captura_por_vehiculo.get(id_vehiculo, 0)
                if ahora - ultima > 5:
                    self.save_vehicle_image(frame, x1, y1, x2, y2, ancho, altura)
                    self.ultima_captura_por_vehiculo[id_vehiculo] = ahora
